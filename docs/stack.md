...
| Миграции | встроенный `RunMigrations` (чтение SQL из `migrations/`) |
...
| Конфиг | `os.Getenv` + helper функции |
| Логи | `log/slog` |
| Тесты | `stretchr/testify` + integration через реальный PG DSN |
...
| Репликация | MVP: HTTP `shard-sync` + shared PG в local/test; streaming replication — target |
